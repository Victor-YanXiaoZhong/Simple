...
// 引用自 https://github.com/ahdung/MsgBox/blob/master/MsgBox.cs
...
namespace Simple.WinUI.Helper
{
    ...
    public static class MessageBoxHelper
    {
        ...
        //提示情景标题
        private const string InfoCaption = "提示...";

        private const string WarningCaption = "警告...";
        private const string ErrorCaption = "错误...";

        /// <summary>是否启用动画效果</summary>
        public static bool EnableAnimate { get; set; } = true;

        /// <summary>是否启用声音反馈</summary>
        public static bool EnableSound { get; set; } = true;

        #region 公开方法

        /// <summary>显示信息框</summary>
        ...
        /// <summary>显示信息框</summary>
        public static void ShowTost(string message, TimeSpan? delayTime = null, string attach = null, string caption = InfoCaption, bool expand = false, string buttonText = null)
        {
            if (delayTime == null)
                delayTime = TimeSpan.FromSeconds(3);
            using var dialog = ShowForm(message, caption, attach, icon: MessageBoxIcon.Information, expand: expand, buttonsText: new[] { buttonText });
            var timer = new System.Threading.Timer((e) =>
            {
                dialog.Close();
            }, null, delayTime.Value, Timeout.InfiniteTimeSpan);
            dialog.ShowDialog();
        }
        ...
        private static MessageForm ShowForm(string message, string caption = null, string attach = null, MessageBoxButtons buttons = MessageBoxButtons.OK, MessageBoxIcon icon = MessageBoxIcon.None, MessageBoxDefaultButton defaultButton = MessageBoxDefaultButton.Button1, bool expand = false, string[] buttonsText = null)
        {
            return new MessageForm(message, caption, buttons, icon, defaultButton, attach, EnableAnimate, EnableSound, expand, buttonsText);
        }
        ...
        private class MessageForm : Form
        {
            ...
            private const int MaxClientWidth = 700; //最大默认窗体客户区宽度
            private static readonly Font GlobalFont = SystemFonts.MessageBoxFont;

            private readonly string _messageSound;
            private readonly bool _expand;
            private readonly bool _useSound;
            private readonly MessageBoxButtons _buttons;
            private readonly bool _hasAttach;
            private readonly ToggleButton _ckbToggle;
            private readonly MessageViewer _msgViewer;
            private readonly PanelBasic _panelButtons;
            private readonly PanelBasic _panelAttach;
            private bool _useAnimate;
            private int _expandHeight;

            /// <summary>创建消息窗体</summary>
            ...
            private static MessageViewer CreateMessageViewer(MessageBoxIcon icon, string text, out string sound)
            {
                Icon ico;
                switch (icon)
                {
            ...
            private static PanelBasic CreateButtonsPanel(bool hasAttach, bool useAnimate, MessageBoxButtons buttons, MessageBoxDefaultButton defaultButton, string[] buttonsText, out Control[] createdButtons, out int defaultButtonIndex)
            {
                // 由于CreateButtonsPanel时仍未加入Form，所以字体尚未继承Form， 此时依赖字体的尺寸计算都不可靠，所以创建按钮时需指定字体
            ...
            private static PanelBasic CreateAttachPanel(string attach)
            {
                var txb = new TextBox
                {
            ...
            /// <summary>改变窗体高度。内部有动画处理</summary>
            ...
            /// <summary>消息呈现控件</summary>
            private class MessageViewer : Control
            {
                private const TextFormatFlags TEXTFLAGS = TextFormatFlags.EndEllipsis        //未完省略号
                ...
                private const int IconSpace = 10; //图标与文本间距
                private const float PreferredScale = 12;//最佳文本区块比例（宽/高）
                ...
                /// <summary>获取或设置图标</summary>
                public Icon Icon { get; set; }

                /// <summary>根据原尺寸，得到相同面积、且指定比例的新尺寸</summary>
                ...
                /// <summary>获取刨去Padding的内容区</summary>
            ...
            /// <summary>包装ToolBarButton为单一控件</summary>
            private class ToggleButton : Control
            {
                /// <summary>展开/收起图标数据</summary>
                private const string ImgDataBase64 =
...
SUVORK5CYII=";
                ...
                private readonly bool isToggleMode;
                private readonly ImageList imgList;
                private bool isChecked;
                private bool useAnimate;

                /// <summary>创建ToolBarButtonControl</summary>
                ...
                /// <summary>Checked改变后</summary>
                public event EventHandler CheckedChanged;

                /// <summary>使用动画按钮效果</summary>
                private bool UseAnimate
                {
                    get => useAnimate;
                    set
                ...
                /// <summary>获取或设置按钮是否处于按下状态</summary>
                ...
                /// <summary>调整按钮尺寸刚好为ToolBar尺寸</summary>
                ...
                /// <summary>引发CheckedChanged事件</summary>
            ...
            // ReSharper restore FieldCanBeMadeReadOnly.Local ReSharper restore MemberCanBePrivate.Local
