...
using thisForm = System.Windows.Forms;
...
namespace Simple.WinUI.Controls.PopupControl
{
        ...
        private static void InitializeGripBitmap(Graphics g, Size size, bool forceRefresh)
        {
            if (m_sGripBitmap == null || forceRefresh || size != m_sGripBitmap.Size)
            {
                ...
                using (Graphics gripG = Graphics.FromImage(m_sGripBitmap))
                    thisForm.ControlPaint.DrawSizeGrip(gripG, SystemColors.ButtonFace, 0, 0, size.Width, size.Height);
            }
        }
    ...
    public class PopupDropDown : ToolStripDropDown
    {
        ...
        protected override void OnSizeChanged(EventArgs e)
        {
            ...
            // When drop-down window is being resized by the user (i.e. not locked), update size of
            // hosted control.
            if (!m_lockedThisSize)
        ...
        public void Show(int x, int y, int width, int height)
        {
            ...
            // Initially hosted control should be displayed within a drop down of 1x1, however its
            // size should exceed the dimensions of the drop-down.
            {
                m_lockedHostedControlSize = true;
                m_lockedThisSize = true;
